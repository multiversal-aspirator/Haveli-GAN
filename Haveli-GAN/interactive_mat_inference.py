#!/usr/bin/env python3
"""
Interactive MAT Model Inference
===============================

Interactive script to run inference using only the MAT (Mask-Aware Transformer) model.
Allows browsing and selecting specific damaged images for restoration using MAT.
"""
...
class InteractiveMAT:
    def __init__(self):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"🚀 MAT Interactive Inference")
        ...
        self.output_dir = "./mat_interactive_results"
        
        os.makedirs(self.output_dir, exist_ok=True)
        print(f"📁 Output directory: {self.output_dir}")
    
    def load_mat_model(self):
        """Load the trained MAT model"""
        print("\n📦 Loading MAT model...")
        
        try:
            self.mat_model = MAT(device=self.device, lr=0.0002)
            checkpoint_path = "sequential_checkpoints_200epochs/MAT_final.pth"
            
            if os.path.exists(checkpoint_path):
                self.mat_model.load_checkpoint(checkpoint_path)
                self.mat_model.generator.eval()
                print(f"✅ MAT model loaded from: {checkpoint_path}")
            else:
                print(f"❌ Checkpoint not found: {checkpoint_path}")
                raise FileNotFoundError(f"MAT checkpoint not found")
    ...
    def run_mat_inference(self, damaged_tensor, mask_tensor):
        """Run MAT inference using the same method as inference_comparison"""
        print("🔄 Running MAT inference...")
        
        try:
            with torch.no_grad():
                # Use the same inference method as in inference_comparison.py
                output = self.mat_model.inference(damaged_tensor, mask_tensor)
    ...
    def save_results(self, damaged_img, mask_img, result_tensor, base_name):
        ...
        try:
            ...
            # Save files
            comparison_path = os.path.join(self.output_dir, f"{base_name}_mat_comparison.png")
            result_path = os.path.join(self.output_dir, f"{base_name}_mat_result.png")
    ...
    def run_batch_inference(self, images):
        """Run MAT inference on all images"""
        ...
        for idx, image_path in enumerate(images):
            ...
            try:
                ...
                # Run MAT inference
                result_tensor = self.run_mat_inference(damaged_tensor, mask_tensor)
    ...
    def run_interactive_session(self):
        ...
        print("\n🎯 MAT Interactive Inference Session")
        print("=" * 50)
...
def main():
    """Main function"""
    try:
        interactive_mat = InteractiveMAT()
        interactive_mat.run_interactive_session()
    except Exception as e:
        print(f"❌ Failed to initialize MAT interactive inference: {e}")
