#!/usr/bin/env python3
"""
Interactive PartialConv Model Inference
=======================================

Interactive script to run inference using only the PartialConv model.
Allows browsing and selecting specific damaged images for restoration using PartialConv.
"""
...
class InteractivePartialConv:
    def __init__(self):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        print(f"🚀 PartialConv Interactive Inference")
        print(f"📱 Using device: {self.device}")
        
        # Load PartialConv model
        self.load_partialconv_model()
        ...
        self.output_dir = "./partialconv_interactive_results"
        
        os.makedirs(self.output_dir, exist_ok=True)
        print(f"📁 Output directory: {self.output_dir}")
    
    def load_partialconv_model(self):
        """Load the trained PartialConv model"""
        print("\n📦 Loading PartialConv model...")
        
        try:
            self.partialconv_model = PartialConvModel(device=self.device, lr=0.0002)
            
            # Try 5-epoch checkpoint first, fallback to 200-epoch if not available
            checkpoint_paths = [
                "partialconv_5epoch_checkpoints/PartialConv_5epochs_final.pth",
                "sequential_checkpoints_200epochs/PartialConv_final.pth"
            ]
            ...
            for checkpoint_path in checkpoint_paths:
                if os.path.exists(checkpoint_path):
                    try:
                        self.partialconv_model.load_checkpoint(checkpoint_path)
                        self.partialconv_model.model.eval()
                        print(f"✅ PartialConv model loaded from: {checkpoint_path}")
                        loaded = True
                        break
            ...
            if not loaded:
                print(f"❌ No valid PartialConv checkpoint found")
                raise FileNotFoundError(f"PartialConv checkpoint not found")
                
        except Exception as e:
            print(f"❌ Failed to load PartialConv model: {e}")
            raise
    ...
    def run_partialconv_inference(self, damaged_tensor, mask_tensor):
        """Run PartialConv inference using the built-in inference method"""
        print("🔄 Running PartialConv inference...")
        
        try:
            with torch.no_grad():
                # Use the built-in inference method
                output = self.partialconv_model.inference(damaged_tensor, mask_tensor)
    ...
    def save_results(self, damaged_img, mask_img, result_tensor, base_name):
        ...
        try:
            ...
            axes[2].set_title('PartialConv Output')
            axes[2].axis('off')
            ...
            # Save files
            comparison_path = os.path.join(self.output_dir, f"{base_name}_partialconv_comparison.png")
            result_path = os.path.join(self.output_dir, f"{base_name}_partialconv_result.png")
    ...
    def run_batch_inference(self, images):
        """Run PartialConv inference on all images"""
        ...
        for idx, image_path in enumerate(images):
            ...
            try:
                ...
                # Run PartialConv inference
                result_tensor = self.run_partialconv_inference(damaged_tensor, mask_tensor)
    ...
    def run_interactive_session(self):
        ...
        print("\n🎯 PartialConv Interactive Inference Session")
        print("=" * 50)
...
def main():
    """Main function"""
    try:
        interactive_partialconv = InteractivePartialConv()
        interactive_partialconv.run_interactive_session()
    except Exception as e:
        print(f"❌ Failed to initialize PartialConv interactive inference: {e}")
